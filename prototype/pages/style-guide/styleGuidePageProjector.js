 ...
 */
const StyleGuidePageProjector = (pageController, pinToElement, contentFilePath) => {
    ...
    const fetchPageContent = async filePath => {
        try {
            ...
            if (response.ok) {
                ...
                const [htmlContent] = dom(content);
                const root = htmlContent.querySelector('#out');
                UseStylesProjector(root);
                const logoDivs = htmlContent.querySelectorAll("div.kolibri-logo-svg");
                const wings   = htmlContent.querySelectorAll(".wing");
                // remove all flatter classes after initial load
                const resetFlatterClasses = () =>
                    setTimeout( _=> wings.forEach( wing => wing.classList.remove("flatter")), 2000);
                resetFlatterClasses();

                logoDivs.forEach( logoDiv => logoDiv.onclick = _ => {
                    wings.forEach( wing => wing.classList.add("flatter"));
                    resetFlatterClasses();
                });
                return htmlContent;
            } else {
