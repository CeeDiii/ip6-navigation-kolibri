...
export { Attribute, QualifiedAttribute,
         ...
         VALID, VALUE, EDITABLE, LABEL, NAME, TYPE, ACTIVE, HASH, ICON, VISITED, LOGO, VISIBLE, ISHOMEPAGE }

/**
 * @typedef {'value'|'valid'|'editable'|'label'|'name'|'type'|'active'|'hash'|'icon'|'visited'|'logo'|'visible'|'ishomepage'} ObservableTypeString
...
/** @type ObservableTypeString */ const ISHOMEPAGE  = "ishomepage";
...
const ModelWorld = () => {
    ...
    const update = (getQualifier, name, observable) => {
        ...
        const key = qualifier + "." + name; // example: "Car.4711.firstname" "VALID" -> "Car.4711.firstname.VALID"
 ...
 * @example
 * const firstNameAttr = QualifiedAttribute("Car.4711.firstname"); // attr is set to existing values, if any.
 */
 ...
 * @example
 * const firstNameAttr = Attribute("Dierk", "Car.4711.firstname");
 */
